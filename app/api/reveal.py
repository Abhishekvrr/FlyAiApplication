...
router = APIRouter(prefix="/api", tags=["Access Control & Reveal"])

# Policy-Based Access Control (PBAC) Allow Lists
ALLOWED_ROLES = {"CUSTOMER_SUPPORT", "AUDITOR", "PRIVACY_ADMIN"}
...
class RevealRequest(BaseModel):
    ...
    actor: str = Field(..., examples=["agent_john"])
    role: str = Field(..., examples=["CUSTOMER_SUPPORT"])
...
def reveal_customer_field(payload: RevealRequest, db: Session = Depends(get_db)):
    """
    PBAC controlled reveal endpoint for authorized PII exception workflows:
    ...
    req_role = payload.role.strip().upper()
    ...
    """)
    customer = db.execute(cust_query, {"customer_id": payload.customer_id}).mappings().first()
    ...
    if not customer:
        ...
        raise HTTPException(
            ...
            detail=f"Customer ID '{payload.customer_id}' not found in protected store."
        )
